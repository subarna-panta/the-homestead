...
export default function SheriffStore() {
  ...
  const handleAction = async (purchaseId: number, action: "delivered" | "denied") => {
    setActing(purchaseId);
    try {
      const res = await api.store.purchaseAction(purchaseId, action);
  ...
  const past = purchases.filter(p => p.status !== "pending");
    ...
    <div className="space-y-10">
      <div className="flex items-center justify-between border-b border-saddle-900 pb-6">
        <div>
          ...
          <h1 className="font-display text-3xl md:text-4xl uppercase tracking-widest text-parchment-200">General Store</h1>
      ...
      {flash && (
        <div className="ledger-panel px-5 py-3 border-gold/40">
          <p className="font-body text-sm text-parchment-300">{flash}</p>
        </div>
      )}
        ...
        <div className="space-y-10">
          ...
          <section>
            <div className="flex items-center gap-3 mb-4">
              <h2 className="font-display text-xl uppercase tracking-widest text-parchment-200">🔔 Pending Redemptions</h2>
              <span className="font-display text-xs px-2 py-0.5" style={{ background: "#8b2e2e", color: "#f9edda" }}>
                {pending.length}
              </span>
            </div>
            {pending.length === 0 ? (
              <div className="ledger-panel px-5 py-6 text-center">
                <p className="font-body text-parchment-600">No pending redemptions right now.</p>
              </div>
            ) : (
                ...
                {pending.map(p => (
                  <div key={p.id} className="px-5 py-4">
                    ...
                    <div className="flex gap-3">
                      <button
                        onClick={() => handleAction(p.id, "delivered")}
                        disabled={acting === p.id}
                        style={{ background: "#4a7c59", color: "#f9edda", border: "2px solid #14532d", padding: "8px 16px", fontFamily: "var(--font-rye)", fontSize: "12px", textTransform: "uppercase", letterSpacing: "0.1em", cursor: "pointer", opacity: acting === p.id ? 0.5 : 1 }}
                      >
                        ✓ Deliver
                      </button>
                      <button
                        onClick={() => handleAction(p.id, "denied")}
                        disabled={acting === p.id}
                        style={{ background: "#8b2e2e", color: "#f9edda", border: "2px solid #7f1d1d", padding: "8px 16px", fontFamily: "var(--font-rye)", fontSize: "12px", textTransform: "uppercase", letterSpacing: "0.1em", cursor: "pointer", opacity: acting === p.id ? 0.5 : 1 }}
                      >
                        ✗ Deny
                      </button>
                    </div>
                ...
                {past.map(p => (
                  <div key={p.id} className="flex items-center justify-between px-5 py-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span style={{
                        fontFamily: "var(--font-rye)", fontSize: "10px", textTransform: "uppercase",
                        letterSpacing: "0.1em", padding: "2px 8px",
                        border: `2px solid ${p.status === "delivered" ? "#4a7c59" : "#8b2e2e"}`,
                        color: p.status === "delivered" ? "#4a7c59" : "#8b2e2e"
                      }}>
